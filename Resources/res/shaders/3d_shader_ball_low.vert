...
void main(void)
{
	...
	v_diffuse = vec3(0.45) + 0.55 * textureCube(u_env_map, N).rgb;
	...
	float brdfD = 0.7;
	v_specular = prefilteredColor * ((F * brdf.x + vec3(brdf.y)) * brdfD + vec3(1.0 - brdfD));

	float fLightCount = float(u_light_count);
	v_colorMultiplier = 0.9 * (lightAreak + vec3(fLightCount)) / (2.0 * vec3(fLightCount)) + vec3(0.1);
}