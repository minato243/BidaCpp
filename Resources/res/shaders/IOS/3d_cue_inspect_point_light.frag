...
vec4 textureCubeLod(samplerCube tex, vec3 uvw, float lod){
...
void main(void)
{
	...
	for(int i = 0; i < u_light_count; i = i + 1)
	{
		...
		float distance = cur_distance/350.0;
	...
	vec3 diffuse = vec3(0.4) * ao * albedo;

	vec3 prefilteredColor = textureCubeLod(u_irradiance_map, normalize(r).xyz, roughness).rgb;
