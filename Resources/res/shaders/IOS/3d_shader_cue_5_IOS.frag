...
vec3 grayscale(vec3 color){
	float g = color.r * 0.3 + color.g * 0.59 + color.b * 0.11;
	return vec3(g);
}
...
vec4 textureCubeLod(samplerCube tex, samplerCube tex2, vec3 uvw, float lod){
	vec3 color1 = grayscale(textureCube(tex, uvw).rgb);
	vec3 color2 = grayscale(textureCube(tex2, uvw).rgb);
...
{
	// Cut off
	int posx = int(gl_FragCoord.x) * 720;
	int posy = int(gl_FragCoord.y) * 1280;
	...
	for(int i = 0; i < u_light_count; i = i + 1)
	{
		...
		vec3 L = v_light_pos_0;
		float cur_distance = v_distance_0;
		
		if(i == 1){
			L = v_light_pos_1;
			cur_distance = v_distance_1;
		}
		else if(i == 2){
			L = v_light_pos_2;
			cur_distance = v_distance_2;
		}
		else if(i == 3){
			L = v_light_pos_3;
			cur_distance = v_distance_3;
		}
		else if(i == 4){
			L = v_light_pos_4;
			cur_distance = v_distance_4;
		}
		else if(i == 5){
			L = v_light_pos_5;
			cur_distance = v_distance_5;
		}
		// square distance for performance
		// float distance2 = dot(light/300.0, light/300.0);
		float distance = cur_distance/170.0;
	...
	vec3 diffuse = (grayscale(textureCube(u_env_map, N).rgb) * 0.4 + 0.6) * albedo;

	vec3 prefilteredColor = textureCubeLod(u_irradiance_map, u_env_map, normalize(r).xyz, roughness).rgb;
	// prefilteredColor = pow(prefilteredColor, vec3(1.0/3.5));
