...
void main(void)
{
	...
	float metallic = texture2D(u_mrao_map, texCoord).r;
	float roughness = texture2D(u_mrao_map, texCoord).g;
	float ao = texture2D(u_mrao_map, texCoord).b;
	...
	prefilteredColor = textureCubeLod(u_irradiance_map, u_irradiance_map_2, u_env_map, normalize(reflect(-V, N)).xyz, roughness * (u_lod_level+2.0)).rgb;
