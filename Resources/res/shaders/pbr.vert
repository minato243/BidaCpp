...
vec4 textureCubeLod(samplerCube tex, samplerCube tex2, samplerCube tex3, vec3 uvw, float lod){
	...
	if(lod < 3.0)
		return mix(textureCube(tex2, uvw), textureCube(tex3, uvw), frac);
	return textureCube(tex3, uvw);
}
...
void main(void)
{
	...
	vec3 albedo = texture2D(CC_Texture0, texCoord).rgb;
	...
	vec3 V = normalize(u_view_pos - worldPos.xyz);
	...
	vec3 Lo = vec3(0.0);
	v_LoDiffuse = vec3(0.0); 
	v_LoSpecular = vec3(0.0);
	...
	for(int i = 0 ; i < u_light_count ; i++){
		...
		v_LoDiffuse += (kD / PI) * u_light_color * falloff * NoL * luminosity;
		v_LoSpecular += specular * u_light_color * falloff  * NoL * luminosity;
		#else
		// Lo += (kD * albedo / PI + specular) * radiance * NdotL;
		v_LoDiffuse += (kD / PI) * radiance * NdotL;
		v_LoSpecular += specular * radiance * NdotL;
		#endif
	...
	vec3 F = fresnelSchlickRoughness(max(dot(N, V), 0.000001), F0, u_roughness);
	// vec3 F = fresnelSchlickRoughness(NdotV, F0, u_roughness);
	vec3 kS = F;
	vec3 kD = 1.0 - kS;
	kD *= 1.0 - u_metallic;

	vec3 diffuse = textureCube(u_env_map, N).rgb;
	...
	v_ambientDiffuse = diffuse * kD * u_ao * (max(dot(N, vec3(0.0, 1.0, 0.0)), 0.0) * 0.3 + 0.7);
	v_ambientSpecular = specular * u_ao * (max(dot(N, vec3(0.0, 1.0, 0.0)), 0.0) * 0.3 + 0.7);
	#else
	// vec3 ambient = vec3(0.06) * albedo * ao;
	v_ambientDiffuse = 0.06 * ao;
	v_ambientSpecular = vec3(0.0);
	#endif
	...
	// v_color = vec4(color, 1.0) * u_color;
}
